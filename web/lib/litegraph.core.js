    ...
    LGraph.prototype.configure = function(data, keep_old) {
        ...
        if (nodes) {
            for (var i = 0, l = nodes.length; i < l; ++i) {
                ...
                // 如果 n_info.title 不为空，那么有可能是中文，需要转换
                if (n_info.title) {
                    n_info.title = decodeURIComponent(n_info.title);
                }
...
*/
    ...
    LGraphNode.prototype.serialize = function() {
        ...
        if (this.title && this.title != this.constructor.title) {
            // o.title = this.title;
            // 中文会乱码，所以先 urlencode 一下
            o.title = encodeURIComponent(this.title);
        }
    ...
    */
    ...
    LGraphCanvas.prototype.pasteFromClipboard = function(isConnectUnselected = false) {
        ...
        for (var i = 0; i < clipboard_info.nodes.length; ++i) {
            ...
            if (node) {
                // 检查 node 的 title 是否存在，如果存在可能为中文，需要转换
                if (node_data.title) {
                    node_data.title = decodeURIComponent(node_data.title);
                }
